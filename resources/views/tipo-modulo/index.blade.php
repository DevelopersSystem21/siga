...
@section('title', 'Tipo Modulo')
@section('content_header')
    <h1 class="m-0 text-dark">Tipo Modulo</h1>
@stop
            ...
            <div class="modal fade" id="modalRegistrar" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <form action="{{route('tipo-modulo.store')}}" method="POST">
                            @csrf
                            <div class="mb-3">
            ...
            <div class="row d-flex justify-content-between align-items-center pr-3">
                ...
                <div class="col-4 d-flex justify-content-end mr-3">
                    <button class="btn btn-primary btn-sm" data-bs-toggle="modal" data-bs-target="#modalRegistrar">
                        <i class="fa fa-circle-plus" aria-hidden="true"></i>Agregar Tipo Modulo</button>
                </div>
            </div>
            <div class="table-responsive">
                <table class="table table-striped table-hover">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>{{$tipo_modulo->nombre}}</td>
                            <td class="d-flex justify-content-end pr-5">
                                <button class="btn btn-primary btn-sm mr-3" data-bs-toggle="modal" 
                                data-bs-target="#modalEditar{{$tipo_modulo->id}}" title="Editar">
                                <i class="fas fa-fw fa-pen" aria-hidden="true"></i>
                                </button>
                                <form id="deleteForm" action="{{ route('tipo-modulo.destroy', $tipo_modulo) }}" method="POST" style="display: inline;">
                                    @csrf
                                    @method('DELETE')
                            ...
                            <!-- Modal -->
                            <div class="modal fade" id="modalEditar{{$tipo_modulo->id}}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                                <div class="modal-dialog">
                                <div class="modal-content">
                                    <div class="modal-header">
                                    ...
                                    <div class="modal-body">
                                        <form action="{{route('tipo-modulo.update',$tipo_modulo)}}" method="POST">
                                            @csrf
                                            @method('put')
                                            <div class="mb-3">
                                                <label for="descripcion" class="form-label">Nombre</label>
                                                <input type="text" class="form-control" id="descripcion" name="nombre"
                                                value="{{$tipo_modulo->nombre}}" autocomplete="off">
                                            </div>
