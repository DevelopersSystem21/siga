...
@section('content_header')
    <h1 class="m-0 text-dark">Tipo Identificacion</h1>
@stop
        ...
        <div class="col-12">
            ...
            <div class="modal fade" id="modalRegistrar" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <form action="{{route('tipo-identificacion.store')}}" method="POST">
                            @csrf
                            <div class="mb-3">
            ...
            <div class="row d-flex justify-content-between align-items-center pr-3">
                ...
                <div class="col-4 d-flex justify-content-end mr-3">
                    <button class="btn btn-primary btn-sm" data-bs-toggle="modal" data-bs-target="#modalRegistrar">
                        <i class="fa fa-circle-plus" aria-hidden="true"></i>Agregar Tipo Identificacion</button>
                </div>
            </div>
            <div class="table-responsive">
                <table class="table table-striped table-hover">
                    ...
                    <tbody>
                        @foreach ($tipoIdentificacion as $tipoIdentific)
                        <tr>
                            <td>{{$tipoIdentific->nombre}}</td>
                            <td class="d-flex justify-content-end pr-5">
                                <button class="btn btn-primary btn-sm mr-3" data-bs-toggle="modal" 
                                data-bs-target="#modalEditar{{$tipoIdentific->id}}" title="Editar">
                                <i class="fas fa-fw fa-pen" aria-hidden="true"></i>
                                </button>
                                <form id="deleteForm" action="{{ route('tipo-identificacion.destroy', $tipoIdentific) }}" method="POST" style="display: inline;">
                                    @csrf
                                    @method('DELETE')
                            ...
                            <!-- Modal -->
                            <div class="modal fade" id="modalEditar{{$tipoIdentific->id}}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                                <div class="modal-dialog">
                                <div class="modal-content">
                                    <div class="modal-header">
                                    ...
                                    <div class="modal-body">
                                        <form action="{{route('tipo-identificacion.update',$tipoIdentific)}}" method="POST">
                                            @csrf
                                            @method('put')
                                            <div class="mb-3">
                                                <label for="descripcion" class="form-label">Nombre</label>
                                                <input type="text" class="form-control" name="nombre"
                                                value="{{$tipoIdentific->nombre}}" autocomplete="off">
                                            </div>    
                                            <div class="modal-footer">
                                                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cerrar</button>
                                                <button type="submit" class="btn btn-primary">Registrar</button>
