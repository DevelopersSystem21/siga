                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <form action="{{route('departamento.store')}}" method="POST">
                            @csrf
                            <div class="mb-3">
                                <label for="descripcion" class="form-label">Codigo Departamento</label>
                                <input type="text" class="form-control" name="coddepa"
                                autocomplete="off">
                            </div>
                            <div class="mb-3">
                                <label for="descripcion" class="form-label">Nombre</label>
                                <input type="text" class="form-control" name="nomdepa"
                                autocomplete="off">
                            </div>
                            <div class="mb-3">
                                <label for="fecha-fin" class="form-label">Pais</label>
            ...
            <div class="row d-flex justify-content-between align-items-center pr-3">
                ...
                <div class="col-4 d-flex justify-content-end mr-3">
                    <button class="btn btn-primary btn-sm" data-bs-toggle="modal" data-bs-target="#modalRegistrar">
                        <i class="fa fa-circle-plus" aria-hidden="true"></i>Agregar Departamento</button>
                </div>
            </div>
                ...
                <table class="table table-striped table-hover">
                    <thead>
                        <tr>
                            ...
                            <th class=".text-dark-emphasis">PAIS</th>
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>{{$departamento->coddepa}}</td>
                            <td>{{$departamento->nomdepa}}</td>
                            <td>{{$departamento->paises->nompais}}</td>
                            
                            <td class="d-flex justify-content-end pr-5">
                                <button class="btn btn-primary btn-sm mr-3" data-bs-toggle="modal" 
                                data-bs-target="#modalEditar{{$departamento->iddep}}" title="Editar">
                                <i class="fas fa-fw fa-pen" aria-hidden="true"></i>
                                </button>
                                <form id="deleteForm" action="{{ route('departamento.destroy', $departamento) }}" method="POST" style="display: inline;">
                                    @csrf
                                    @method('DELETE')
                            ...
                            <!-- Modal -->
                            <div class="modal fade" id="modalEditar{{$departamento->iddep}}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                                <div class="modal-dialog">
                                <div class="modal-content">
                                    <div class="modal-header">
                                    ...
                                    <div class="modal-body">
                                        <form action="{{route('departamento.update',$departamento)}}" method="POST">
                                            ...
                                            <div class="mb-3">
                                                <label for="descripcion" class="form-label">Codigo Departamento</label>
                                                <input type="text" class="form-control" name="coddepa"
                                                value="{{$departamento->coddepa}}" autocomplete="off">
                                            </div>
                                            <div class="mb-3">
                                                <label for="descripcion" class="form-label">Nombre</label>
                                                <input type="text" class="form-control" name="nomdepa"
                                                value="{{$departamento->nomdepa}}" autocomplete="off">
                                            </div>
                                            <div class="mb-3">
                                                <label for="fecha-fin" class="form-label">Pais</label>
                                                <select class="form-select" name="idpais" id="idpais">
                                                    <option selected>Select Option</option>
                                                    @foreach ($paises as $pais)
                                                    <option value="{{$pais->idpais}}" @if($pais->idpais == $departamento->idpais) selected @endif>
                                                        {{$pais->nompais}}</option>
                                                    @endforeach
                                                  </select>
                                            </div>    
