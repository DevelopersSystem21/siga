...
class NumerosEnLetras
{
    ...
    public static function convertirSimple($number)
    {
        return strtoupper(self::$UNIDADES[abs($number)]);
    }
    public static function convertir($number, $currency = '', $format = false, $decimals = '')
    {
        ...
        $converted = rtrim(strtoupper($converted));
        if ($format) {
            if (empty($decimales)) {
                $valor_convertido = "SON {$converted} CON 00/100 {$currency}";
            } else {
                $valor_convertido = "SON {$converted} CON {$decNumberStr}/100 {$currency}";
            }
        } else {
            if (empty($decimales)) {
                $valor_convertido = strtoupper($converted) . $currency;
            } else {
                $valor_convertido = strtoupper($converted) . $currency . ' con ' . $decimales . $decimals;
            }
        }
